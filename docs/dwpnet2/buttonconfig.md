...
Each button inside `DWProx Net2+` **->** `Buttons` will have a `ButtonSettings` [ModuleScript](https://create.roblox.com/docs/reference/engine/classes/ModuleScript), which will look as follows:
...
Changes the text on the prompt. If `nil`, it will use a system text. 
